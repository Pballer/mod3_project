...
return either "Reject the null hypothesis" or
"Fail to reject the null hypothesis" depending on the specified alpha
"""
...
def hypothesis_test_one(alpha, clean_data):
    ...
    team_wins = clean_data.loc[clean_data.game_season > 2008,
    ...
    ttest_results = stats.ttest_ind(home_teams.game_home_team_score,
                                    away_teams.game_visitor_team_score,
                                    equal_var=False)
    print(ttest_results)
    ...
    print(f'Based on the p value of {p_val} and our aplha of {alpha} we {status.lower()}  the null hypothesis.'
          ...
          f'is a difference between total points scored at home vs away.')
...
def hypothesis_test_three(alpha, clean_data):
    ...
    print(f'Based on the p value of {p_val} and our aplha of {alpha} we {status.lower()}  the null hypothesis.'
          f'\n Due to these results, we  {assertion} state that there is a difference '
          f'between the Bucks performance between 2017 and 2018 seasons.')
